...
<head>
    ...
    <script type="text/javascript">
        ...
        function saveUser () {
            if (checkForm()) {
                var url = '${base}/user/addUser';
                var userId = $("#userId").val();
                if (!isEmpty(userId)) {
                    var url = '${base}/user/editUser';
                }
                $.ajax({
                    type: "post",
                    url: url,
...
<nav class="breadcrumb pngfix">
    ...
    <span class="c_gray en">&gt;</span><a href="javascript:changeSrc('${base}/user/index')">人员管理</a>
            ...
            <td>
                <select id="roleId" name="roleId" class="select">
                    ...
                    <#list roleList as role>
                        <option <#if role.roleId! == transUser.roleId!>selected="selected" </#if> value="${role.roleId!}">${role.roleName!}</option>
                    </#list>
                </select>
                </select>
            </td>
    ...
    <div class="row-fluid">
        <div class="span10 offset2">
          <button type="button" class="btn btn-primary" id="btnCA">导入CA证书</button>
            ...
            <button type="button" class="btn" onclick="javascript:changeSrc('${base}/user/index?userType=0')">返回</button>
        </div>
    </div>
</form>
<@Ca autoSign=0  />
