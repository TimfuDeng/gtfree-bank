...
<head>
    ...
    <script type="text/javascript">
        ...
        function checkForm(){
            /*if (!checkInputNull('qualifiedAuditor','审核人必须填写!'))
                return false;*/
//            if (!checkInputNull('bankCode','银行必须填写!'))
//                return false;
            if(!checkInputNull('qualifiedStatus','是否通过必须填写!'))
                return false;
        ...
        function pass () {
            if (checkForm()) {
                var url = '${base}/qualified/verify/save';
...
<form class="form-base" method="post" action="${base}/qualified/verify/save">
    ...
    <#if transResourceApply.applyType??&&transResourceApply.applyType==1&&transUserUnionList??>
        ...
        <tr>
            ...
            <td width="300px">
                <input type="text" name="qualifiedTime" class="input-text Wdate"
                       ...
                       onfocus="WdatePicker({firstDayOfWeek:1,dateFmt:'yyyy-MM-dd HH:mm',startDate:'%y-%M-%d 09:00:00',minDate:'%y-%M-%d %H:%m:%s',alwaysUseStartDate:true})">
            </td>
        ...
        <tr>
            ...
            <td>
                <input  type="radio" name="qualifiedStatus" value="1" <#if transBuyQualified?? && transBuyQualified.qualifiedStatus==1 >checked="true"  </#if> />是
                <input type="radio" name="qualifiedStatus" value="2" <#if transBuyQualified?? && transBuyQualified.qualifiedStatus==2 >checked="true" </#if> />否
            </td>
        ...
        <tr id="qualifiedContacter">
            <td><label class="control-label">联系方式：</label></td>
            <td colspan="3"><input type="text" name="contacter" class="input-text"></td>
        </tr>
    ...
    <div class="row-fluid">
        <div class="span10 offset2">
            <button type="button" class="btn btn-primary" onclick="pass();">保存</button>
            <button type="button" class="btn" onclick="javascript:changeSrc('${base}/qualified/resourceApply?resourceId=${transResourceApply.resourceId!}')">返回</button>
        </div>
    </div>
</form>
