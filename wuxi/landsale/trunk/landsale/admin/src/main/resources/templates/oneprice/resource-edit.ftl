...
//            return true;
        ...
        function submitForm() {
            $.ajax({
                url:"${base!}/oneprice/resource/save",//提交地址
...
<form class="form-base" id="form1" method="post" action="${base!}/oneprice/resource/save">
    <table class="table table-border table-bordered table-striped">
        <tbody><tr>
            <td colspan="4" style="text-align: center;font-size: 14px;font-weight:700">设置报价信息（单位：万元）</td>
        </tr>
        <tr>
            <td width="180"><label class="control-label">宗地编号：</label></td>
            <td>${transTargetName!}</td>
            <td><label class="control-label">市场指导价：</label></td>
        ...
        <tr>
            <td><label class="control-label">最小有效报价：</label></td>
            <td style="mso-number-format: "#,##0.0000">
                <input type="text" class="input-text" name="priceMin" value="<#if oneTarget??>${oneTarget.priceMin!}</#if>">
            </td>
            <td><label class="control-label">最大有效报价：</label></td>
            <td style="mso-number-format: "#,##0.0000">
        ...
        <input type="hidden"  name="transResourceId" value="${transResourceId!}">
