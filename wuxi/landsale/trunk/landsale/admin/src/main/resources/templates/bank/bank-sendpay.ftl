...
<head>
    ...
    <script type="text/javascript">
        ...
        function submitForm () {
            $.ajax({
                type: "post",
                url: "${base}/bank/sendPay",
                data: $("form").serialize(),
                success: function (data) {
                    if(data.code == "00"){
                        _alertResult('ajaxResultDiv', true, "发送成功！");
                    } else {
                        _alertResult('ajaxResultDiv', false, data.message);
                    }
                }
            });
        }
...
<form class="form-base" method="post" action="${base}/console/bank/sendpay">
    ...
    <div class="row-fluid">
        <div class="span10 offset2">
            <button type="button" class="btn btn-primary" onclick="submitForm()" >发送</button>
