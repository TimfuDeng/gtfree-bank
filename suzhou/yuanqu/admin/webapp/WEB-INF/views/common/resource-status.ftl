...
<#elseif resource.resourceEditStatus==3 >
    ...
    <#if resource.suspendNoticeId??>
        <a href="${base}/console/suspendNotice/detail?noticeId=${resource.suspendNoticeId}" class="btn size-S btn-primary">查看中止公告</a>
    <#else>
        <a href="${base}/console/suspendNotice/editByResourceCode?resourceId=${resource.resourceId}" class="btn size-S btn-primary">新增中止公告</a>
    </#if>
    <input class="btn size-S btn-secondary" type="button" value="退回编辑" onclick="BackEdit('${resource.resourceId!}')">
...
<#elseif resource.resourceEditStatus==4 || resource.resourceEditStatus==9>
    ...
    <#if resource.dealNoticeId??>
        <a href="${base}/console/dealNotice/detail?noticeId=${resource.dealNoticeId!}" class="btn btn-primary">查看成交公示</a>
    <#else>
        <a href="${base}/console/dealNotice/editByResourceCode?resourceId=${resource.resourceId!}" class="btn btn-primary">新增成交公示</a>
    </#if>
    </#if>
