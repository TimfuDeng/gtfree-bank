...
public class TransUserUnionServiceImpl extends HibernateRepo<TransUserUnion, String> implements TransUserUnionService {
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<TransUserUnion> findTransUserUnionByUserNameIsAgree(String userName, Pageable request) {
        Map<String,Object> mapParam= Maps.newHashMap();
        mapParam.put("userName", userName);
        String hql = "from TransUserUnion t where t.userName=:userName and t.agree=0 order by t.unionId desc";
        return findByHql(hql, mapParam, request);
    }
