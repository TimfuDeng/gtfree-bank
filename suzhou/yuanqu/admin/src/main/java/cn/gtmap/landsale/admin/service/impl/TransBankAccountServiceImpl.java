...
public class TransBankAccountServiceImpl extends HibernateRepo<TransBankAccount, String> implements TransBankAccountService {
    ...
    @Override
    @Transactional(readOnly = true)
    public TransBankAccount getTransBankAccountByAccountCodeAndApplyNo(String accountCode, String applyNo) {
        return get(criteria(Restrictions.eq("applyNo", applyNo)).add(Restrictions.eq("accountCode", accountCode)));
    }
