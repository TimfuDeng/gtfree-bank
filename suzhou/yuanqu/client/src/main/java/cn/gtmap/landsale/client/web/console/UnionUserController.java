...
public class UnionUserController {
    ...
    public @ResponseBody String edit(String applyId,TransUserUnion transUserUnion,Model model){
        if (null==transUserService.getTransUserByUserName(transUserUnion.getUserName())) {
            throw new AppException(3100);
        }
       ...
       /* double scale=0;
        ...
        if(StringUtils.isEmpty(transUserUnion.getUnionId()))
            transUserUnion.setUnionId(null);
    ...
    public @ResponseBody String agree(String unionId){
        ...
        if(null!=oldTransResourceApply&&oldTransResourceApply.getApplyStep()>1)
