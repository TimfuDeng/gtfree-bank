...
<div class="wp">
    <nav class="breadcrumb pngfix">
        ...
        <a href="${base}/index/type" class="maincolor">首页</a>
...
<div class="row" style="border: 1px solid #eeecec;overflow: hidden">
                    ...
                    <table class="table table-info">
                        <tr>
                            <td>
                                ...
                                <span class="current-price">¥${resource.beginOffer!}&nbsp;
                                <#if resource.offerUnit?? && resource.offerUnit==0 >万元</#if>
                               <#-- <#if resource.offerUnit?? && resource.offerUnit==1 >元/平方米（单价）</#if>
                                <#if resource.offerUnit?? && resource.offerUnit==2 >万元/亩（单价）</#if>-->
                                </span>
                            </td>
                            <td>
                                <span class="title">加价幅度：</span>
                                <span>¥${resource.addOffer!}&nbsp;
                                <#if resource.offerUnit?? && resource.offerUnit==0 >万元</#if>
                              <#--  <#if resource.offerUnit?? && resource.offerUnit==1 >元/平方米（单价）</#if>
                                <#if resource.offerUnit?? && resource.offerUnit==2 >万元/亩（单价）</#if>-->
                                </span>
                        ...
                        <tr>
                            <td>
                                <span class="title">限时报价开始时间：</span>
    ...
    <div class="col-2 cl" style="border-left: 1px solid #e9e9e9;height: 495px;
        ...
        <h5 style="border-bottom: 1px solid #eeecec;line-height:40px">竞买记录
            <span id="applyNumber"><#include "common/view-applynumber.ftl"></span>
        </h5>
        <table class="table table-price">
            <thead>
                <tr>
                    <td style="width:48px;">状态</td>
                    <td style="width:55px;">号牌</td>
                    <td>报价</td>
                </tr>
            </thead>
            <tbody>
            ...
            <tr>
                <td>
                    <#if resourceOffer_index==0>
                        ...
                        <span class="label label-default label-max">领先</span>
                    <#else>
                ...
                <td >
                    <#if userId?? && resourceOffer.userId==userId>
                        <span class="label label-danger ">我的报价</span>
                    <#else>
        ...
        <script id="tpl_beginoffer" type="text/html">
            ...
            <p style='color: #d91615;font-size: 20px'>大写：{{ d.offerChinese }}元整</p>
                ...
                {{# if(d.offerNoteType==1){ }}
                <span><span style='color: red'>*</span> 您的报价已是当前最高价，是否继续报价？ </span>
                {{# }else if(d.offerNoteType==2){ }}
                <span ><span style='color: red'>*</span> 您的价格超过竞价增幅5倍，是否继续报价？</span>
                {{# } }}
            </div>
        </script>
...
<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-footer">
        <button class="btn btn-primary" id="btn_modal" onclick="javascript:postOffer();">确定</button> <button class="btn" data-dismiss="modal" aria-hidden="true">取消</button>
    </div>
</div>

<div id="myModalLimit" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-header">
        <h4 id="myModalLabel"><i class="icon-warning-sign"></i>参与限时报价确认对话框</h4><a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a>
    </div>
    <div class="modal-body">
        <p id="modal_content2">您是否确认参加限时报价！</p>
    </div>
    <div class="modal-footer">
        <button class="btn btn-primary" id="btn_modal" onclick="javascript:confirmLimit('${resource.resourceId}');">确定</button> <button class="btn" data-dismiss="modal" aria-hidden="true">取消</button>
    </div>
</div>
...
<#else>
    ...
    <script src="${base}/static/js/slide.js"></script>
    <script src="${base}/static/thridparty/H-ui.2.0/lib/bootstrap-modal.js"></script>
    <script src="${base}/static/thridparty/H-ui.2.0/lib/bootstrap-modalmanager.js"></script>
    <script src="${base}/static/thridparty/laytpl/laytpl.js"></script>
    <script src="${base}/static/js/index.js"></script>
    <script src="${base}/static/js/view.js"></script>
