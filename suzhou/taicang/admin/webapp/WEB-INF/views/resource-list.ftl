                    ...
                    if(data.ret!=null&&data.ret==true){
                        try {
                            if (_status == 3) {
                                $.ajax({
                                    type: 'POST',
                                    url: '${base}/oneprice/resource/status/change.f',
                                    data: {
                                        "transTargetId": _resourceId,
                                        "status": 1
                                    },
                                    success: function (responseObj) {
                                        if(responseObj.ret!=null&&responseObj.ret==true){

                                        } else {
                                            alert('一次报价发布失败，' + responseObj.msg);
                                        }
                                    }
                                });
                            }
...
<div class="search_bar">
    <div class="navbar-inner">
        <form class="navbar-form" id="frmSearch">
            <div class="l">
                <select id="queryType" name="queryType" class="select" style="width: 100px;height: 31px;">
                    <option value="bianhao">编号</option>
                    <option value="weizhi">位置</option>
                    <option value="mianji">面积</option>
                </select>
                <input type="text" class="input-text" style="width:200px" name="title" value="${title!}" placeholder="请输入查询数据">
...
<table class="table" style="table-layout: fixed;width: 100%;border-collapse: collapse;border-spacing: 2px;border-color: gray;">
    <thead>
        <tr class="text-c" style="background-color: #f5f5f5">
            ...
            <th style="width:280px;border-right: 1px solid #e8e8e8;">挂牌和报名起止时间</th>
            <th style="width:137px;border-right: 1px solid #e8e8e8;">起始价（万元）</th>
            <th style="width:137px">交易状态</th>
        </tr>
    </thead>
    <tbody>
    <#list transResourcePage.items as resource>
        ...
        <tr class="order-bd">
            ...
            <td>
                <p>
                    <em style="font-family: verdana;font-style: normal;">${resource.gpBeginTime?string("yyyy-MM-dd HH:mm:ss")}</em>~<em style="font-family: verdana;font-style: normal;">${resource.gpEndTime?string("yyyy-MM-dd HH:mm:ss")}</em>
                </p>
                <p>
                    <em style="font-family: verdana;font-style: normal;">${resource.bmBeginTime?string("yyyy-MM-dd HH:mm:ss")}</em>~<em style="font-family: verdana;font-style: normal;">${resource.bmEndTime?string("yyyy-MM-dd HH:mm:ss")}</em>
                </p>
            </td>
            ...
            <td>
                ...
                <#else>
                    <#if resource.qualificationType??&&resource.qualificationType=='PRE_TRIAL'>
                        <#if zgqsAuthorize>
                            <a target="_blank" href="${base}/console/resource-apply/list?resourceId=${resource.resourceId}" class="btn btn-primary size-S">资格审核</a>
                        </#if>
                    </#if>
                </#if>
            </td>
