...
public class TransUserServiceImpl extends HibernateRepo<TransUser, String> implements TransUserService {
    ...
    @Override
    @Transactional(readOnly = true)
    public TransUser getTransUserByKeyInfo(String userName, String thumbprint) {
        TransUser transUser;
        if (StringUtils.isNotBlank(thumbprint)) {
            transUser = getTransUserByCAThumbprint(thumbprint);
            if (transUser != null)
                return transUser;
        }

        if (StringUtils.isNotBlank(userName)) {
            transUser = getTransUserByUserName(userName);
            if (transUser != null)
                return transUser;

            String suffixName = getSuffixCode(userName);
            if (StringUtils.isNotBlank(suffixName)) {
                List<TransUser> transUsers = list(criteria(Restrictions.like("userName", "%" + suffixName)));
                if (transUsers != null && transUsers.size() == 1)
                    return transUsers.get(0);
            }
        }
        return null;
    }
