    ...
    <script type="text/javascript">
           ...
           function checkForm(){
               if (!checkInputNull('waitTime','等待时长必须填写!'))
                   return false;
               if (!checkInputNull('queryTime','询问时长必须填写!'))
                   return false;
               if (!checkInputNull('priceTime','报价时长必须填写!'))
                   return false;
               if (!checkInputNumber('waitTime','等待时长必须是数字!'))
                   return false;
               if (!checkInputNumber('queryTime','询问时长必须是数字!'))
                   return false;
               if (!checkInputNumber('priceTime','报价时长必须是数字!'))
                   return false;
               if (!checkInputInteger('waitTime','等待时长必须正整数!'))
                   return false;
               if (!checkInputInteger('queryTime','询问时长必须正整数!'))
                   return false;
               if (!checkInputInteger('priceTime','报价时长必须正整数!'))
                   return false;

               var waitTime = $("input[name='waitTime']").val();
               var queryTime = $("input[name='queryTime']").val();
               var priceTime = $("input[name='priceTime']").val();
               return true;
           }
           function checkInputNumber(name,info){
               if(isNaN($.trim($("input[name='"+name+"']").val()))){
           ...
           function checkInputInteger(name,info){
               if($.trim($("input[name='"+name+"']").val())!="" && $.trim($("input[name='"+name+"']").val())<=0 ){
                   $("input[name='"+name+"']").addClass("error_input");
                   $("input[name='"+name+"']").focus();
                   alert(info);
                   return false;
               }else if($.trim($("input[name='"+name+"']").val()).indexOf(".")>=0){
                   $("input[name='"+name+"']").addClass("error_input");
                   $("input[name='"+name+"']").focus();
                   alert(info);
                   return false;
...
<form class="form-base" method="post" action="${base!}/oneprice/between/save">
    ...
    <div class="row-fluid">
        <div class="span10 offset2">
            ...
            <button type="button" class="btn" onclick="window.location.href='/admin/oneprice/between/list'">返回</button>
        </div>
    </div>
</form>
</body>
