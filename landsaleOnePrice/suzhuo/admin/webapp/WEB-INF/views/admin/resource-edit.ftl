    ...
    <script type="text/javascript">
        function checkForm(){
            ...
            if (!checkInputNumber('priceMax','最大有效报价必须是数字!'))
                return false;
            if (!checkInputInteger('priceGuid','场指导价必须正整数!'))
                return false;
            if (!checkInputInteger('priceMin','最小有效报价必须正整数!'))
                return false;
            if (!checkInputInteger('priceMax','最大有效场报价必须正整数!'))
                return false;
        ...
        function checkInputInteger(name,info){
            if($("input[name='"+name+"']").val().trim()!="" && $("input[name='"+name+"']").val().trim()<=0 ){
                ...
                alert(info);
                return false;
            }else if($("input[name='"+name+"']").val().trim().indexOf(".")>=0){
                $("input[name='"+name+"']").addClass("error_input");
                $("input[name='"+name+"']").focus();
                alert(info);
                return false;
...
<form class="form-base" method="post" action="${base!}/oneprice/resource/save">
    <table class="table table-border table-bordered table-striped">
        ...
        <#assign goods=ResultUtil.getTransGoods(transTargetId)!>
            <input type="hidden"  name="transArea" value="${goods.area2Extend!}">
            <input type="hidden"  name="transUseLand" value="${goods.landUse2Extend!}">
            <input type="hidden"  name="transAddress" value="${goods.address2Extend!}">
